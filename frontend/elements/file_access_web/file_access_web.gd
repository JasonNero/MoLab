# Adapted from https://github.com/Scrawach/godot-file-access-web
...
signal error()
signal download_completed(file_name: String)

var _file_uploading: JavaScriptObject
var _file_downloading: JavaScriptObject
...
var _on_file_download_complete_callback: JavaScriptObject
...
func _init() -> void:
    ...
    _file_downloading = JavaScriptBridge.get_interface("godotFileAccessWebDownload")
    ...
    _on_file_download_complete_callback = JavaScriptBridge.create_callback(_on_file_download_complete)
    ...
    _file_downloading.setDownloadCompleteCallback(_on_file_download_complete_callback)
...
func download(file_name: String, file_data: String) -> void:
    if _is_not_web():
        _notify_error()
        return

    _file_downloading.download(file_name, file_data)
...
func _on_file_download_complete(args: Array) -> void:
    var file_name: String = args[0]
    download_completed.emit(file_name)
...
function godotFileAccessWebStart() {
    ...
    var interface = {
        ...
        setDownloadCompleteCallback: (complete) => downloadCompleteCallback = complete,

        setAcceptFiles: (files) => input.setAttribute("accept", files),
        open: () => input.click(),
        download: (fileName, fileData) => {
            var element = document.createElement('a');
            element.setAttribute('href', 'data:text/plain;base64,' + fileData);
            element.setAttribute('download', fileName);
            element.style.display = 'none';
            document.body.appendChild(element);
            element.click();
            document.body.removeChild(element);
            downloadCompleteCallback(fileName);
        }
    }
...
var godotFileAccessWebDownload = godotFileAccessWebStart();
"""
