...
class InferenceArgs(GenerateOptions, CondSyntOptions, CustomSyntOptions):
    ...
    Attributes:
        text_prompt: Input text for motion generation.
        num_samples: The number of motion samples to generate. Default is 3.
        packed_motion: A mapping of frame indices to poses (J+1, 3), where each
            pose is defined by the root position and all J joint rotations.
            When no motion is supplied it will be initialized as "empty" motion.
            Alternatively you can use `bvh_path` to load a motion from file.
        unpack_mode: Mode for unpacking and converting `packed_motion` to features.
            Choices are "step" (stepped animation) and "linear" (interpolation).
            Default is "linear".
        unpack_randomness: Randomness applied during unpacking. Default is 0.0.
        editable_features: Indicates which features of the input are observed.
            Default is "pos_rot_vel", alternatives are "pos_rot" or just "pos".
        bvh_path: Path to the BVH file. Needs to be used in combination with
            `edit_mode` to specify the masking. Can not be used together with
            `packed_motion`.
        edit_mode: Masking strategy for the BVH input motion. For all options
            see `parser_util.CondSyntOptions`.
        jacobian_ik: Flag to switch from Basic to Jacobian IK. Default is False.
        foot_ik: Flag to enable Foot IK to reduce foot sliding. Default is False.
        imputate: Flag to enable imputation between inference steps. Default is False.
        reconstruction_guidance: Flag to enable reconstruction guidance during
...
class MotionInferenceWorker:
    ...
    def infer(
        ...
        """Infer using InferenceArgs and return InferenceResults, optionally
        save intermediate results to output directory.

        Args:
            infer_config (InferenceArgs): The arguments for motion inference.
            save_results (bool, optional): Whether to save outputs to disk. Defaults to True.

        Returns:
            InferenceResults: The generated motion.
        """
        ###########################################################################
